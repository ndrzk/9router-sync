...
const UNIX_JSON_PATH = join(HOME, '.9router', 'db.json');
...
/**
 * Detect database format on Unix systems.
 * Checks for JSON first, then SQLite.
 * @returns {'json' | 'sqlite' | null} Database format, or null if neither exists
 */
export function detectDatabaseFormat() {
  if (existsSync(UNIX_JSON_PATH)) {
    return 'json';
  }
...
/**
 * Merge two arrays of provider connection rows by id.
 * Preserves existing rows not present in incoming, adds all incoming rows.
 * Incoming rows win on ID conflicts (mimics SQLite UPSERT behavior).
 * @param {Array<object>} existingRows - Current rows (in flat Windows-JSON format)
 * @param {Array<object>} incomingRows - New rows to merge (in flat Windows-JSON format)
 * @returns {Array<object>} Merged array of rows
 */
export function mergeRowsById(existingRows, incomingRows) {
  const incomingIds = new Set(incomingRows.map(r => r.id));
  return [
    ...existingRows.filter(r => !incomingIds.has(r.id)),
    ...incomingRows
  ];
}
...
export function readProviderConnections() {
  ...
  if (platformType === 'windows') {
    ...
    if (format === 'json') {
      ...
      throw new Error(`No 9router database found. Checked:\n  ${UNIX_JSON_PATH}\n  ${UNIX_DB_PATH}`);
    }
  }
}
...
/**
 * Read from Unix JSON database.
 * @returns {Array<object>} Normalized rows
 */
function readUnixJson() {
  if (!existsSync(UNIX_JSON_PATH)) {
    throw new Error(`Unix JSON database not found: ${UNIX_JSON_PATH}`);
  }
  
  let data;
  try {
    data = JSON.parse(readFileSync(UNIX_JSON_PATH, 'utf8'));
  } catch (e) {
    throw new Error(`Failed to parse Unix JSON database: ${e.message}`);
  }
  
  if (!data.providerConnections || !Array.isArray(data.providerConnections)) {
    throw new Error('Invalid Unix JSON database: missing or invalid providerConnections array');
  }
...
function readUnixite() {
  if (!existsSync(UNIX_DB_PATH)) {
    throw new Error(`Unix SQLite database not found: ${UNIX_DB_PATH}`);
  }
...
function writeWindowsJson(rows) {
  ...
  if (existsSync(WINDOWS_DB_PATH)) {
    try {
      ...
      throw new Error(`existing DB JSON is unparseable; refusing to write to avoid data loss: ${e.message}`);
    }
  }
  
  const incomingRows = rows.map(normalizedToWindowsRow);
  const mergedRows = mergeRowsById(existingData.providerConnections || [], incomingRows);
  
  const newData = {
    ...existingData,
    providerConnections: mergedRows,
  };
  
  try {
    const tmpPath = WINDOWS_DB_PATH + '.tmp';
    writeFileSync(tmpPath, JSON.stringify(newData, null, 2), 'utf8');
    renameSync(tmpPath, WINDOWS_DB_PATH);
  } catch (e) {
...
/**
 * Write to Unix JSON database.
 * @param {Array<object>} rows - Normalized rows
 */
function writeUnixJson(rows) {
  // Read existing DB to preserve other data
  let existingData = {};
  if (existsSync(UNIX_JSON_PATH)) {
    try {
      existingData = JSON.parse(readFileSync(UNIX_JSON_PATH, 'utf8'));
    } catch (e) {
      throw new Error(`existing DB JSON is unparseable; refusing to write to avoid data loss: ${e.message}`);
    }
  }
  ...
  try {
    ...
    throw new Error(`Failed to write Unix JSON database: ${e.message}`);
  }
}
